<template>
  <div class="orders">
    <div v-for="(order,index) in this.allOfOrders" v-bind:key="order.id" class="order">
...
export default {
  name: "receptionLookup",
  components:{
    OrderInformation
  },
  computed:{
    allOfOrders(){
      return this.$store.state.allOfOrders;
    }
  },
  methods: {
    sampleAddress(index){
      let temp = this.allOfOrders[index];
